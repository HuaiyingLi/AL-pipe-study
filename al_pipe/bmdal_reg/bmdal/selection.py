...
class IterativeSelectionMethod(SelectionMethod):
    ...
    def __init__(
        self, pool_features: Features, train_features: Features, sel_with_train: bool, verbosity: int = 1, **config
    ):
        """
        ...
        self.selected_arr = torch.zeros(
            self.pool_features.get_n_samples(), dtype=torch.bool, device=self.pool_features.get_device()
        )
...
class ForwardBackwardSelectionMethod(IterativeSelectionMethod, ABC):
    ...
    def __init__(
        ...
        sel_with_train: bool,
        verbosity: int = 1,
        overselection_factor: float = 1.0,
        **config,
    ):
        super().__init__(
    ...
    def select(self, batch_size: int) -> torch.Tensor:
        ...
        for i in range(overselect_batch_size - batch_size):
            ...
            if next_idx is None or next_idx < 0 or next_idx >= len(self.selected_idxs):
                self.status = (
                    f"removing the latest overselected samples because the backward step failed "
...
class RandomSelectionMethod(SelectionMethod):
    ...
    def select(self, batch_size: int) -> torch.Tensor:
        ...
        return torch.randperm(
            self.pool_features.get_n_samples(), device=self.pool_features.get_device(), generator=generator
        )[:batch_size]
...
class MaxDetSelectionMethod(IterativeSelectionMethod):
    ...
    def __init__(
        ...
        **config,
        ...
        """
        super().__init__(
            pool_features=pool_features, train_features=train_features, sel_with_train=sel_with_train, **config
        )
    ...
    def prepare(self, n_adds: int):
        ...
        new_l = torch.zeros(
            len(self.features), n_total, device=torch.device(self.features.get_device()), dtype=self.diag.dtype
        )
...
class BaitFeatureSpaceSelectionMethod(ForwardBackwardSelectionMethod):
    ...
    def __init__(
        ...
        **config,
        ...
        self.scores_numerator = torch.einsum(
            "ij,ji->i", self.feature_matrix, self.feature_cov_matrix @ self.feature_matrix.t()
        )
...
class MaxDistSelectionMethod(IterativeSelectionMethod):
    ...
    def __init__(self, pool_features: Features, train_features: Features, sel_with_train: bool = True, **config):
        super().__init__(
            pool_features=pool_features, train_features=train_features, sel_with_train=sel_with_train, **config
        )
    ...
    def __init__(
        ...
        **config,
        ...
        self.min_sq_dists = np.inf * torch.ones(
            self.pool_features.get_n_samples(), dtype=pool_features.get_dtype(), device=pool_features.get_device()
        )
        self.closest_idxs = torch.zeros(
            self.pool_features.get_n_samples(), device=pool_features.get_device(), dtype=torch.long
        )
        self.neg_inf_tensor = torch.as_tensor(
            [-np.inf], dtype=pool_features.get_dtype(), device=pool_features.get_device()
        )
...
class FrankWolfeSelectionMethod(IterativeSelectionMethod):
    ...
    def __init__(self, pool_features: Features, train_features: Features, sel_with_train: bool = False, **config):
        super().__init__(
            pool_features=pool_features, train_features=train_features, sel_with_train=sel_with_train, **config
        )
        if pool_features.get_n_features() <= 0:
            raise NotImplementedError(
                "Frank-Wolfe is currently only implemented for finite-dimensional feature maps "
    ...
    def add(self, new_idx: int):
        ...
        self.current_embedding = (
            1 - gamma
        ) * self.current_embedding + gamma * self.sqrt_diag_sum * normalized_feature_vector
...
class FrankWolfeKernelSpaceSelectionMethod(IterativeSelectionMethod):
    ...
    def __init__(self, pool_features: Features, train_features: Features, sel_with_train: bool = False, **config):
        ...
        self.u = torch_cat(
            [self.features[start:stop].get_kernel_matrix(self.features).sum(dim=-1) for start, stop in batch_intervals],
            dim=0,
        )
...
class RandomizedMinDistSumSelectionMethod(MaxDistSelectionMethod):
    ...
    def get_next_idx(self) -> Optional[int]:
        ...
        weights = torch.ones_like(self.min_sq_dists) if self.n_added == 0 else torch.clamp(self.min_sq_dists, min=0.0)
...
class SumOfSquaredDistsSelectionMethod(MaxDistSelectionMethod):
    ...
    def get_scores(self) -> torch.Tensor:
        ...
        return -torch_cat(
            [
                torch.minimum(
                    self.min_sq_dists[None, :].expand(stop - start, -1),
                    self.pool_features[start:stop].get_sq_dists(self.pool_features),
                ).sum(dim=1)
