...
def select_batch(
    ...
    data: Dict[str, FeatureData],
    y_train: Optional[torch.Tensor],
    base_kernel: str,
    kernel_transforms: List[Tuple[str, List]],
    selection_method: str,
    precomp_batch_size: int = 32768,
    nn_batch_size=8192,
    **config,
    ...
    return bs.select(
        ...
        precomp_batch_size=precomp_batch_size,
        ...
        **config,
        ...
        if base_kernel == "ntk":  # data -> features
            feature_maps = [
                ReLUNTKFeatureMap(
                    n_layers=config.get(
                        "n_ntk_layers", len([l for l in model.modules() if isinstance(l, LayerGradientComputation)])
                    ),
                    sigma_w_sq=config.get("weight_gain", 0.4) ** 2,
                    sigma_b_sq=config.get("sigma_b", 0.0) ** 2,
                )
            ...
            feature_maps = [
                ReLUNNGPFeatureMap(
                    n_layers=config.get(
                        "n_nngp_layers", len([l for l in model.modules() if isinstance(l, LayerGradientComputation)])
                    ),
            ...
            for model in self.models:
                ...
                for layer in model.modules():
                    if isinstance(layer, LayerGradientComputation):
                        ...
                        grad_layers.append(grad_dict[type(layer)](layer))
                feature_maps.append(
                    create_grad_feature_map(model, grad_layers[-n_last_layers:], use_float64=use_float64)
                )
        elif base_kernel == "linear":
            feature_maps = [
                IdentityFeatureMap(n_features=self.data["train"].get_tensor(0).shape[-1]) for model in self.models
            ]
            if use_float64:
                self.to_float64()
        ...
        self.features = {
            key: [Features(fm, feature_data) for fm in feature_maps] for key, feature_data in self.data.items()
        }
        ...
        if selection_method == "random":
            ...
            if (
                config.get("allow_maxdet_fs", False)
                and n_features > 0
                and n_features * 4 < n_select
                and maxdet_sigma > 0.0
            ):
                alg = MaxDetFeatureSpaceSelectionMethod(
                    self.features["pool"], self.features["train"], noise_sigma=maxdet_sigma, **config
                )
            else:
                alg = MaxDetSelectionMethod(
                    self.features["pool"], self.features["train"], noise_sigma=config.get("maxdet_sigma", 0.0), **config
                )
        elif selection_method == "bait":
            alg = BaitFeatureSpaceSelectionMethod(
                self.features["pool"], self.features["train"], noise_sigma=config.get("bait_sigma", 0.0), **config
            )
            ...
            alg = RandomizedMinDistSumSelectionMethod(
                self.features["pool"],
                self.features["train"],
                max_n_candidates=config.get("max_rmds_candidates", 5),
                **config,
            )
        ...
        results_dict = {
            "kernel_time": kernel_timer.get_result_dict(),
