...
class DataTransform:
    ...
    def __call__(self, feature_data: FeatureData, idxs: Optional[Indexes] = None) -> FeatureData:
        ...
        pieces = [self.forward(sub_data, sub_idxs) for sub_idxs, sub_data in feature_data.iterate(idxs)]
...
class FeatureMap(DataTransform):
    ...
    def posterior(
        self, feature_data: FeatureData, sigma: float, allow_kernel_space_posterior: bool = True
    ) -> "FeatureMap":
        """
    ...
    def get_feature_matrix(self, feature_data: FeatureData, idxs: Optional[Indexes] = None) -> torch.Tensor:
        ...
        idxs = Indexes(feature_data.get_n_samples(), idxs)
        return torch_cat(
            [self.get_feature_matrix_impl_(sub_data, sub_idxs) for sub_idxs, sub_data in feature_data.iterate(idxs)],
            dim=-2,
        )

    def get_kernel_matrix(
        self,
        feature_data_1: FeatureData,
        feature_data_2: FeatureData,
        idxs_1: Optional[Indexes] = None,
        idxs_2: Optional[Indexes] = None,
    ) -> torch.Tensor:
        """
        ...
        return torch_cat(
            [
                torch_cat(
                    [
                        self.get_kernel_matrix_impl_(sub_data_1, sub_data_2, sub_idxs_1, sub_idxs_2)
                        for sub_idxs_2, sub_data_2 in feature_data_2.iterate(idxs_2)
                    ],
                    dim=-1,
                )
                for sub_idxs_1, sub_data_1 in feature_data_1.iterate(idxs_1)
            ],
            dim=-2,
        )
    ...
    def get_kernel_matrix_diag(self, feature_data: FeatureData, idxs: Optional[Indexes] = None) -> torch.Tensor:
        ...
        return torch_cat(
            [
                self.get_kernel_matrix_diag_impl_(sub_data, sub_idxs)
    ...
    def get_kernel_matrix_impl_(
        self, feature_data_1: FeatureData, feature_data_2: FeatureData, idxs_1: Indexes, idxs_2: Indexes
    ) -> torch.Tensor:
        """
...
class LaplaceKernelFeatureMap(FeatureMap):
    ...
    def get_kernel_matrix_impl_(
        ...
        sq_dist_mat = (
            (feature_mat_1**2).sum(dim=-1)[:, None]
            + (feature_mat_2**2).sum(dim=-1)[None, :]
...
class KernelSpacePosteriorFeatureMap(FeatureMap):
    ...
    def precompute_soft_(self, feature_data: FeatureData, idxs: Indexes) -> Tuple["FeatureMap", FeatureData]:
        ...
        return PrecomputedKernelSpacePosteriorFeatureMap(self.feature_map), ListFeatureData(
            [feature_data[idxs].simplify(), TensorFeatureData(tensor)]
        )
...
class PrecomputedKernelSpacePosteriorFeatureMap(FeatureMap):
    ...
    def get_kernel_matrix_impl_(
        ...
        prior_mat = self.feature_map.get_kernel_matrix(
            feature_data_1.feature_data_list[0], feature_data_2.feature_data_list[0], idxs_1, idxs_2
        )
...
class ScaledFeatureMap(FeatureMap):
    ...
    def __init__(self, feature_map: FeatureMap, factor: float):
        ...
        """
        super().__init__(
            n_features=feature_map.get_n_features(), allow_precompute_features=feature_map.allow_precompute_features
        )
        self.feature_map = feature_map
        self.factor = factor
...
class SumFeatureMap(FeatureMap):
    ...
    def __init__(self, feature_maps: List[FeatureMap]):
        ...
        super().__init__(
            n_features=sum(feature_counts) if all([n >= 0 for n in feature_counts]) else -1,
    ...
    def get_kernel_matrix_impl_(
        ...
        return sum(
            [
                fm.get_kernel_matrix(fd1, fd2, idxs_1, idxs_2)
                for fm, fd1, fd2 in zip(
                    self.feature_maps, feature_data_1.feature_data_list, feature_data_2.feature_data_list
                )
            ]
        )
...
class ProductFeatureMap(FeatureMap):
    ...
    def __init__(self, feature_maps: List[FeatureMap]):
        ...
        allow_precompute_features = (
            has_finitely_many_features
            and (len([fc for fc in feature_counts if fc != 1]) <= 1)
            and all([fm.allow_precompute_features for fm in feature_maps])
        )
        super().__init__(
            n_features=utils.prod(feature_counts) if has_finitely_many_features else -1,
    ...
    def get_feature_matrix_impl_(self, feature_data: FeatureData, idxs: Indexes) -> torch.Tensor:
        ...
        slices = [
            [slice(None)] + [None] * i + [slice(None)] + [None] * (len(self.feature_maps) - 1 - i)
            for i in range(len(self.feature_maps))
        ]
        matrix_long = utils.prod(
            [
                fm.get_feature_matrix(fd, idxs)[s]
...
class ElementwiseSumFeatureMap(FeatureMap):
    ...
    def __init__(self, feature_maps: List[FeatureMap]):
        ...
        """
        super().__init__(
            n_features=feature_maps[0].get_n_features(),
            allow_precompute_features=all([fm.allow_precompute_features for fm in feature_maps]),
        )
        self.feature_maps = feature_maps
...
class ElementwiseProductFeatureMap(FeatureMap):
    ...
    def get_feature_matrix_impl_(self, feature_data: FeatureData, idxs: Indexes) -> torch.Tensor:
        if not isinstance(feature_data, ListFeatureData):
            raise ValueError(f"feature_data must be of type ListFeatureData, but is of type {type(feature_data)}")
        return self.n_features ** ((len(self.feature_maps) - 1) / 2) * utils.prod(
            [fm.get_feature_matrix(fd, idxs) for fm, fd in zip(self.feature_maps, feature_data.feature_data_list)]
        )
...
class LinearFeatureMap(FeatureMap):
    ...
    def sketch(self, n_features: int, **config) -> "FeatureMap":
        ...
        return SequentialFeatureMap(IdentityFeatureMap(n_features=self.get_n_features()), [self]).sketch(
            n_features, **config
        )
