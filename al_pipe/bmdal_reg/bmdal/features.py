...
class Features:
    ...
    def posterior_tfm(
        self, sigma: float = 1.0, allow_kernel_space_posterior: bool = True, **config
    ) -> "FeaturesTransform":
        """
        ...
        """
        fm = self.feature_map.posterior(
            self.feature_data, sigma, allow_kernel_space_posterior=allow_kernel_space_posterior
        )
    ...
    def acs_rf_hyper_tfm(self, y_train: torch.Tensor, n_features: int, **config) -> "FeaturesTransform":
        """
        ...
        theta_samples = L_inv.t().matmul(
            torch.randn(in_features, n_features, device=x_train.device, dtype=x_train.dtype)
        )

        acs_rf_tfm = ACSRFHyperDataTransform(
            theta_cov=theta_cov, theta_samples=theta_samples, a_tilde=a_tilde, b_tilde=b_tilde, y_var=y_var
        )
        fm = SequentialFeatureMap(IdentityFeatureMap(n_features=n_features), [self.feature_map, acs_rf_tfm])
        return LambdaFeaturesTransform(lambda f, fm=fm: Features(fm, f.feature_data))

    def acs_rf_tfm(self, n_features: int, sigma: float = 1.0, **config) -> "FeaturesTransform":
        """
    ...
    def __getitem__(self, idxs: Union[int, slice, torch.Tensor]) -> "Features":
        ...
        return Features(
            self.feature_map, self.feature_data[idxs], None if self.diag is None else self.diag[idxs.get_idxs()]
        )
...
class ACSRFHyperDataTransform(DataTransform):
    ...
    def __init__(
        self, theta_cov: torch.Tensor, theta_samples: torch.Tensor, a_tilde: float, b_tilde: float, y_var: float
    ):
        super().__init__()
