    ...
    def __init__(self, n_samples: int, idxs: Optional[Union[torch.Tensor, slice, int, "Indexes"]]):
        ...
        if idxs is None:
            ...
            if idxs.step is not None and idxs.step != 1:
                raise ValueError("Cannot handle slices with step size other than 1")
            ...
            if stop <= start:
                raise ValueError("stop <= start not allowed for slices")
...
class TensorFeatureData(FeatureData):
    ...
    def simplify_multi_(self, feature_data_list: List["TensorFeatureData"], idxs_list: List[Indexes]) -> "FeatureData":
        return TensorFeatureData(
            torch_cat([fd.data[idxs.get_idxs()] for fd, idxs in zip(feature_data_list, idxs_list)], dim=0)
        )
...
class ConcatFeatureData(FeatureData):
    ...
    def __init__(self, feature_data_list: List[FeatureData]):
        ...
        super().__init__(
            ...
            device=feature_data_list[0].get_device(),
    ...
    def simplify_impl_(self, idxs: Indexes) -> "FeatureData":
        simplified = [
            self.feature_data_list[i].simplify(sub_idxs) for i, sub_idxs in idxs.split_by_sizes(self.sample_sizes)
        ]
        ...
        if len(simplified) == 1:
            ...
            if utils.all_equal([type(fd) for fd in simplified]):
                ...
                return simplified[0].simplify_multi_(
                    simplified, [Indexes(fd.get_n_samples(), None) for fd in simplified]
                )
            else:
    ...
    def simplify_multi_(self, feature_data_list: List["ConcatFeatureData"], idxs_list: List[Indexes]) -> "FeatureData":
        ...
        return ConcatFeatureData(
            [
                SubsetFeatureData(fd, idxs)
                for cfd, idxs in zip(feature_data_list, idxs_list)
                for fd in cfd.feature_data_list
            ]
        ).simplify()
...
class ListFeatureData(FeatureData):  # does not concatenate along batch dimension
    ...
    def __init__(self, feature_data_list: List[FeatureData]):
        ...
        """
        super().__init__(
            n_samples=feature_data_list[0].get_n_samples(),
            device=feature_data_list[0].get_device(),
            dtype=feature_data_list[0].get_dtype(),
        )
        self.feature_data_list = feature_data_list
    ...
    def simplify_multi_(self, feature_data_list: List["ListFeatureData"], idxs_list: List[Indexes]) -> "FeatureData":
        ...
        return ListFeatureData(
            [
                ConcatFeatureData(
                    [fd.feature_data_list[i][idxs] for fd, idxs in zip(feature_data_list, idxs_list)]
                ).simplify()
