from bmdal_reg.layers import ParallelLinearLayer, ParallelSequential, get_parallel_act_layer
...
def create_tabular_model(n_models, n_features, hidden_sizes=[512] * 2, act="relu", n_outputs: int = 1, **config):
    ...
    layers.append(
        ParallelLinearLayer(
            ...
            layer_sizes[-2],
            layer_sizes[-1],
            weight_init_mode="zero" if config.get("use_llz", False) else "normal",
            **config,
        )
    )
