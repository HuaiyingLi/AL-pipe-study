...
def getSubfolderNames(folder):
    return [os.path.basename(name) for name in os.listdir(folder) if os.path.isdir(os.path.join(folder, name))]
...
def prod(it: Iterable, id=None):
    ...
    if result is None:
        if id is None:
            raise ValueError("Cannot compute empty product without identity element")
        else:
            return id
    return result
