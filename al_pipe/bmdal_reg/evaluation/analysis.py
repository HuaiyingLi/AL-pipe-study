...
class ExperimentResults:
    ...
    def map_single_split(self, f: Callable):
        ...
        return ExperimentResults(
            {
                alg_name: {
                    task_name: [f(split) for split in split_results]
    ...
    def get_learning_curves(self, key: str) -> "ExperimentResults":
        return ExperimentResults(
            {
                alg_name: {
                    task_name: np.mean(np.log([split["errors"][key] for split in split_results]), axis=0)
                    for task_name, split_results in task_results.items()
                    if int(task_name.split("_")[-1].split("x")[0]) == 256
                }
                for alg_name, task_results in self.results_dict.items()
            },
            exp_name=self.exp_name,
    ...
    def get_learning_curves_splits(self, key: str) -> "ExperimentResults":
        ...
        return ExperimentResults(
            {
                alg_name: {
                    task_name: [np.log(split["errors"][key]) for split in split_results]
                    for task_name, split_results in task_results.items()
                    if int(task_name.split("_")[-1].split("x")[0]) == 256
                }
                for alg_name, task_results in self.results_dict.items()
    ...
    def get_avg_al_stats(self, key: str) -> "ExperimentResults":
        return self.map_single_split(lambda split_dict: [stat[key] for stat in split_dict["al_stats"] if key in stat])
    ...
    def get_avg_errors(self, key: str, use_log: bool = True) -> "ExperimentResults":
        if use_log:
            return self.map_single_split(lambda split_dict: np.mean(np.log(split_dict["errors"][key][1:])))
        else:
            return self.map_single_split(lambda split_dict: np.mean(split_dict["errors"][key][1:]))
    ...
    def get_average_al_times(self) -> "ExperimentResults":
        return self.map_single_split(
            lambda split_dict: np.mean(
                [sr["kernel_time"]["total"] + sr["selection_time"]["total"] for sr in split_dict["al_stats"]]
            )
        )
    ...
    def select_split(self, i: int) -> "ExperimentResults":
        ...
        return ExperimentResults(
            {
                alg_name: {task_name: split_results[i] for task_name, split_results in task_results.items()}
                for alg_name, task_results in self.results_dict.items()
            },
            exp_name=self.exp_name,
        )
    ...
    def filter_task_suffix(self, task_suffix: str) -> "ExperimentResults":
        return ExperimentResults(
            {
                alg_name: {
                    task_name: task_dict for task_name, task_dict in alg_dict.items() if task_name.endswith(task_suffix)
                }
                for alg_name, alg_dict in self.results_dict.items()
            },
            exp_name=self.exp_name,
        )
        ...
        return ExperimentResults(
            {
                alg_name: {
                    task_name: task_dict
                    for task_name, task_dict in alg_dict.items()
    ...
    def filter_task_names(self, task_names: List[str]) -> "ExperimentResults":
        return ExperimentResults(
            {
                alg_name: {task_name: task_dict for task_name, task_dict in alg_dict.items() if task_name in task_names}
                for alg_name, alg_dict in self.results_dict.items()
            },
            exp_name=self.exp_name,
        )
    ...
    def filter_alg_names(self, alg_names: Iterable[str]) -> "ExperimentResults":
        return ExperimentResults(
            {alg_name: self.results_dict[alg_name] for alg_name in alg_names if alg_name in self.results_dict},
            exp_name=self.exp_name,
        )

    def filter_common_algs(self) -> "ExperimentResults":
        common_alg_names = [
            alg_name for alg_name, alg_dict in self.results_dict.items() if set(alg_dict.keys()) == set(self.task_names)
        ]
                ...
                for split_idx, split_result in enumerate(split_results):
                    for al_stat_idx, al_stat in enumerate(split_result["al_stats"]):
                        ...
                        if "selection_status" in al_stat and al_stat["selection_status"] is not None:
                            print(
                                f"Alg {alg_name} failed on step {al_stat_idx} of split {split_idx} of task {task_name}:",
    ...
    def load(exp_name: str) -> "ExperimentResults":
        ...
        if utils.existsFile(pkl_filename) and os.path.getmtime(pkl_filename) >= utils.last_mod_time_recursive(
            str(results_path)
        ):
            try:
                ...
                print(
                    f"Received exception while trying to load cached results, "
...
def save_latex_table_all_algs(results: ExperimentResults, filename: str):
    ...
    alg_metrics = {
        alg_name: {
            name: np.mean([np.mean(metric.results_dict[alg_name][ds_name + "_256x16"]) for ds_name in ds_names])
            for name, metric in metrics_and_names
        }
    ...
    for metric_name, _ in metrics_and_names:
        ...
        best_alg_names_per_metric[metric_name] = [
            all_alg_names[i] for i in range(len(all_alg_names)) if rounded_results[i] == min_result
        ]

    table_rows = {}
    table_header = (
        "\\begin{tabular}{cccccccc}\n"
        + " & ".join([r"Selection method", r"Kernel", r"MAE", r"RMSE", r"95\%", r"99\%", r"MAXE", r"avg.\ time [$s$]"])
        + "\\\\\n\\hline\n"
    )
    table_footer = "\n\\end{tabular}"
    ...
    sub_groups = [
        [alg_name for alg_name in all_alg_names if raw_sel_names[alg_name] == sel_name] for sel_name in sel_name_order
    ]
...
def save_latex_table_data_sets(
    ...
    alg_metrics = {
        alg_name: {ds_name: np.mean(rmse.results_dict[alg_name][ds_name + "_256x16"]) for ds_name in ds_names}
    ...
    for ds_name in ds_names:
        ...
        min_result = np.min(rounded_results)
        best_alg_names_per_data_set[ds_name] = [
            alg_names[i] for i in range(len(alg_names)) if rounded_results[i] == min_result
        ]
    ...
    table_header = (
        ...
        + ("c" * (len(alg_names) + 1))
        + "}\n"
        + " & ".join([r"Data set"] + [get_latex_selection_method(alg_name.split("_")[1]) for alg_name in alg_names])
        + "\\\\\n\\hline\n"
    )
    table_footer = "\n\\end{tabular}"
...
def print_single_task_results(exp_results: ExperimentResults, task_name: str):
    ...
    for metric_name in metric_names:
        ...
        alg_stds = {
            alg_name: np.std(results[alg_name][task_name]) / np.sqrt(len(results[alg_name][task_name]))
            for alg_name in alg_names
        }
...
def print_avg_results(exp_results: ExperimentResults, relative_to: Optional[str] = None, filter_suffix: str = "256x16"):
    """
    ...
    relative_to_name = (
        ...
        else (relative_to if relative_to in alg_names else [name for name in alg_names if relative_to in name][0])
    )
    ...
    for metric_name in metric_names:
        ...
        avg_results = exp_results.get_avg_errors(metric_name)
        alg_task_means = {
            alg_name: {task_name: np.mean(avg_results.results_dict[alg_name][task_name]) for task_name in task_names}
            for alg_name in alg_names
        }
        if relative_to is not None:
            alg_task_means = {
                alg_name: {
                    task_name: alg_task_means[alg_name][task_name] - alg_task_means[relative_to_name][task_name]
        ...
        if relative_to is not None:
            # take std over tasks
            alg_stds = {
                alg_name: np.std(list(task_means.values())) / np.sqrt(len(list(task_means.values())))
                for alg_name, task_means in alg_task_means.items()
            }
        else:
            # take sd over splits
            alg_stds = {
                alg_name: np.linalg.norm(
                    [
                        np.std(avg_results.results_dict[alg_name][task_name])
