...
from bmdal_reg.evaluation.analysis import (
    ExperimentResults,
    get_latex_ds_name,
    get_latex_metric_name,
    get_latex_selection_method,
    get_latex_task,
)
...
colors = ["#a06010", "#d62728", "#e377c2", "#2ca02c", "#ff7f0e", "#9467bd", "#17becf", "#7f7f7f", "#bcbd22", "#1f77b4"]
markers = ["^", "v", "<", ">", "P", "D", "s", "v"]


def plot_batch_sizes_ax(
    ax: plt.Axes, results: ExperimentResults, metric_name: str, set_ticks_and_labels: bool = True, **plot_options
):
    ...
    for alg_name in results.alg_names:
        ...
        for ds_name in ds_names:
            ...
            task_stds = [
                np.std(last_errors.results_dict[alg_name][key], axis=0)
                / np.sqrt(len(last_errors.results_dict[alg_name][key]) - 1)
                for key in keys
            ]
        ...
        if alg_name == "NN_random":
            ...
            ax.axhline(
                y=np.mean(log_means), ls="--", color="k", label=get_latex_selection_method(alg_name.split("_")[1])
            )
        else:
            ...
            ax.plot(
                np.log(ds_batch_sizes[0]),
                np.mean(log_means, axis=0),
                "--",
...
def plot_batch_sizes(
    results: ExperimentResults, filename: str | Path, metric_name: str, figsize: tuple[float, float] | None = None
):
...
def plot_multiple_batch_sizes(results: ExperimentResults, filename: str | Path, metric_names: list[str]):
...
def plot_batch_sizes_individual(results: ExperimentResults, metric_name: str):
    ...
    for ds_name in ds_names:
        ...
        plt.tight_layout()
        plot_name = (
            Path(custom_paths.get_plots_path()) / results.exp_name / "ds_batch_sizes" / f"{ds_name}_{metric_name}.pdf"
        )
        utils.ensureDir(plot_name)
        plt.savefig(plot_name)
        plt.close(fig)
...
def plot_batch_sizes_individual_subplots(results: ExperimentResults, filename: str | Path, metric_name: str):
    # plot mean final log metric against the al batch size
    ds_names = list({"_".join(task_name.split("_")[:-1]) for task_name in results.task_names})
    ds_names.sort()
    ...
    for ds_idx, ds_name in enumerate(ds_names):
        ...
        ax = axs[i, j]
        plot_batch_sizes_ax(
            ax, results.filter_task_prefix(ds_name), metric_name, set_ticks_and_labels=False, markersize=3.5
        )
...
def plot_batch_sizes_metrics_subplots(results: ExperimentResults, filename: str | Path):
    # plot mean final log metric against the al batch size
...
def plot_learning_curves_ax(
    ax: plt.Axes,
    results: ExperimentResults,
    metric_name: str,
    with_random_final: bool = True,
    ...
    **plot_options,
    ...
    for i, alg_name in enumerate(results.alg_names):
        ...
        alg_ds_ntrain_log_values = np.asarray(
            [log_values.results_dict[alg_name][ds_name + "_256x16"] for ds_name in ds_names]
        )
        ...
        # shape: n_train_steps
        # see Appendix E.4
        std_errors = np.linalg.norm(np.std(alg_ds_ntrain_log_values, axis=1) / np.sqrt(n_splits - 1), axis=0) / len(
            ds_names
        )
        ...
        if alg_name == "NN_random":
            if with_std_errors:
                ax.fill_between(np.log(n_train), means - std_errors, means + std_errors, facecolor="k", alpha=0.2)
            ax.plot(np.log(n_train), means, "--o", color="k", label=label, **plot_options)
            if with_random_final:
                ax.plot(
                    [np.log(n_train[0]), np.log(n_train[-1])], [means[-1], means[-1]], "--", color="k", **plot_options
                )
        else:
            if with_std_errors:
                ax.fill_between(
                    np.log(n_train), means - std_errors, means + std_errors, facecolor=colors[color_idx], alpha=0.2
                )
            ax.plot(
                np.log(n_train),
                means,
...
def plot_learning_curves(
    ...
    plot_learning_curves_ax(
        axs, results=results, metric_name=metric_name, labels=labels, with_std_errors=with_std_errors
    )
...
def plot_multiple_learning_curves(
    results: ExperimentResults, filename: str | Path, metric_names: list[str], with_std_errors: bool = False
):
    # plot averaged learning curve for all tasks
    fig, axs = plt.subplots(1, len(metric_names), figsize=(7, 4))
...
def plot_learning_curves_individual(results: ExperimentResults, metric_name: str, with_std_errors: bool = False):
    ...
    for ds_name in ds_names:
        ...
        plot_learning_curves_ax(
            axs,
            results.filter_task_names([ds_name + "_256x16"]),
        ...
        plt.tight_layout()
        plot_name = (
            Path(custom_paths.get_plots_path())
            / results.exp_name
            / "ds_learning_curves"
...
def plot_learning_curves_individual_subplots(
    results: ExperimentResults, filename: str, metric_name: str, with_std_errors: bool = False
):
    # plot individual learning curve for all tasks
    ds_names = list({"_".join(task_name.split("_")[:-1]) for task_name in results.task_names})
    ds_names.sort()
...
def plot_learning_curves_metrics_subplots(
    results: ExperimentResults, filename: str | Path, with_std_errors: bool = False
):
    # plot mean final log metric against the al batch size
...
def plot_correlation_between_methods(results: ExperimentResults, filename: str | Path, metric_name: str):
    ...
    all_results = {
        alg_name: {task_name: np.mean(last_errors.results_dict[alg_name][task_name]) for task_name in task_names}
        for alg_name in alg_names + ["NN_random"]
    }
            ...
            for task_name in task_names:
                ...
                if i == 0 and j == 1:
                    axs[j - 1, i].plot(
                        ...
                        "o",
                        alpha=alpha,
                        label=get_latex_task(task_name.split("_256")[0]),
                    ...
                    axs[j - 1, i].plot(
                        result_x, result_y, "o", markersize=markersize, alpha=alpha, color=c[k], markeredgewidth=0.0
                    )
                    k += 1
...
def plot_skewness_ax(
    ax: plt.Axes, results: ExperimentResults, metric_name: str, alg_name: str, use_relative_improvement: bool = False
):
    sns.color_palette("Paired")
    ...
    all_results = {
        name: {task_name: np.mean(avg_errors.results_dict[name][task_name]) for task_name in task_names}
        for name in [alg_name, random_name]
    }
    ...
    for task_idx, task_name in enumerate(task_names):
        ...
        first_step_results = {
            metric: np.mean([np.log(random_results[i]["errors"][metric][0]) for i in range(n_splits)])
        ...
        ax.set_xlabel("mean log RMSE - mean log MAE", **axis_font)
...
def plot_error_variation(
    ...
    plot_skewness_ax(
        ax, results, metric_name=metric_name, alg_name=alg_name, use_relative_improvement=use_relative_improvement
    )
...
def plot_eff_dim(results: ExperimentResults, filename: str | Path, pattern: str, metric_name: str):
    ...
    for name_task in ds_names:
        ...
        for key in last_errors.results_dict[all_alg_names[0]]:
            if key.startswith(name_task) and int(key.split("_")[-1].split("x")[0]) == 256:
                log_means_ll = [
                    np.mean(last_errors.results_dict[all_alg_names[0]][key]),
                    np.mean(eff_dims.results_dict[all_alg_names[0]][key]),
                ]
                log_means_grad_rp = [
                    np.mean(last_errors.results_dict[all_alg_names[1]][key]),
        ...
        axs.plot(
            log_means_grad_rp[1] - log_means_ll[1], log_means_ll[0] - log_means_grad_rp[0], "o", label=escape(name_task)
        )
...
def plot_learning_curves_v2(results: ExperimentResults, filename: str | Path, metric_name: str):
    # plot averaged learning curve for all tasks
    all_alg_names = list(results.results_dict.keys())
    all_task_names = {task_name for alg_name in all_alg_names for task_name in results.results_dict[alg_name].keys()}
    ds_names = list({"_".join(task_name.split("_")[:-1]) for task_name in all_task_names})
    algs_with_all_tasks = [
        alg_name for alg_name in all_alg_names if set(results.results_dict[alg_name].keys()) == all_task_names
    ]
    ...
    alg_results = {
        alg_name: np.mean([learning_curves.results_dict[alg_name][ds_name + "_256x16"] for ds_name in ds_names], axis=0)
...
def alt_plot_eff_dim(results: ExperimentResults, filename: str | Path, pattern: str, metric_name: str):
    # plots, for every data set, error of grad_rp-512 to ll vs difference in effective dimensions (average),
    # for different selection methods (esp. maxdist, maxdet, lcmd, fw),
    ...
    for name_task in ds_names:
        ...
        for key in last_errors.results_dict[all_alg_names[0]]:
            if key.startswith(name_task) and int(key.split("_")[-1].split("x")[0]) == 256:
                ...
                log_means_random = [
                    np.mean(last_errors.results_dict[all_alg_names[2]][key]),
        ...
        axs.plot(
            log_means_grad_rp[1] - log_means_ll[1],
            (log_means_ll[0] - log_means_grad_rp[0])
            / (abs(log_means_grad_rp[0] - log_means_random[0]) + abs(log_means_ll[0] - log_means_random[0])),
            "o",
            label=escape(name_task),
        )
