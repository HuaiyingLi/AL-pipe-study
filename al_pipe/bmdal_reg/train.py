...
from .data import ParallelDictDataLoader, TaskSplit
...
class ModelTrainer:
    ...
    def __call__(self, task_split: TaskSplit, device: str, do_timing: bool = False):
        ...
        print(
            f"Running {self.alg_name} on split {task_split.id} of task {task_split.task_name} on device {device}",
            flush=True,
        )
        ...
        if base_kernel is None:
            ...
            for al_step, al_batch_size in enumerate(task_split.al_batch_sizes):
                print(
                    ...
                    f", n_pool={len(pool_idxs)}, al_batch_size={al_batch_size}",
                    flush=True,
                )
                ...
                feature_data = {"train": X[train_idxs], "pool": X[pool_idxs]}
                y_train = data.tensors["y"][train_idxs].to(al_device)

                al_timer.start()
                new_idxs, al_stats = select_batch(
        ...
        results = {
            "errors": {key: [r[key] for r in results] for key in ["mae", "rmse", "maxe", "q95", "q99"]},
...
def test_model(model, data, n_models, test_idxs):
    test_dl = ParallelDictDataLoader(
        data, test_idxs.expand(n_models, -1), batch_size=8192, shuffle=False, adjust_bs=False, drop_last=False
    )
    with torch.no_grad():
        model.eval()
...
def fit_model(
    ...
    **config,
    ...
    train_dl = ParallelDictDataLoader(
        data, train_idxs.expand(n_models, -1), batch_size=batch_size, shuffle=True, adjust_bs=False, drop_last=True
    )
    if do_valid:
