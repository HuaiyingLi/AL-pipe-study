...
class RunConfigList:
    ...
    def __init__(self, configs: list[tuple[float, ModelTrainer, float]] | None = None):
        """
        Constructor.
    ...
    def filter_names(self, alg_names: list[str]):
        ...
        return RunConfigList(
            [
                (ram_gb_per_sample, trainer, ram_gb_per_sample_float)
...
def get_bmdal_configs(**kwargs) -> RunConfigList:
    ...
    lst.append(
        1e-6, ModelTrainer("NN_random", selection_method="random", base_kernel="linear", kernel_transforms=[], **kwargs)
    )
    ...
    for fb_mode, overselection_factor in [("f", 1.0), ("fb", 2.0)]:
        ...
        lst.append(
            2e-5,
            ModelTrainer(
                f"NN_bait-{fb_mode}-p_grad_rp-512_train",
                selection_method="bait",
                overselection_factor=overselection_factor,
                base_kernel="grad",
        ...
        if fb_mode == "f":
            lst.append(
                2e-5,
                ModelTrainer(
                    f"NN_bait-{fb_mode}-p_linear_train",
                    selection_method="bait",
                    overselection_factor=overselection_factor,
                    base_kernel="linear",
                    kernel_transforms=[("train", [sigma, None])],
            ...
            lst.append(
                2e-5,
                ModelTrainer(
                    f"NN_bait-{fb_mode}-p_grad_ens-3_rp-512_train",
                    selection_method="bait",
                    overselection_factor=overselection_factor,
                    base_kernel="grad",
                    n_models=3,
                    kernel_transforms=[("ens", []), ("rp", [512]), ("train", [sigma, None])],
                    **kwargs,
                ),
            )
            lst.append(
                2e-5,
                ModelTrainer(
                    f"NN_bait-{fb_mode}-p_ll_ens-3_rp-512_train",
                    selection_method="bait",
                    overselection_factor=overselection_factor,
                    n_models=3,
                    base_kernel="ll",
    ...
    lst.append(
        2.5e-5,
        ModelTrainer(
            "NN_maxdet-p_grad_ens-3_rp-512_train",
            selection_method="maxdet",
            base_kernel="grad",
            n_models=3,
            kernel_transforms=[("ens", []), ("rp", [512]), ("train", [sigma, None])],
    ...
    lst.append(
        2.5e-5,
        ModelTrainer(
            "NN_maxdet-p_grad_rp-512_acs-rf-512",
            selection_method="maxdet",
            base_kernel="grad",
    ...
    lst.append(
        2.5e-5,
        ModelTrainer(
            "NN_maxdet-p_grad_rp-512_acs-grad",
            selection_method="maxdet",
            base_kernel="grad",
    ...
    lst.append(
        2.5e-5,
        ModelTrainer(
            "NN_maxdet-p_grad_rp-512_acs-rf-hyper-512",
            selection_method="maxdet",
            base_kernel="grad",
    ...
    lst.append(
        2e-5,
        ModelTrainer(
            "NN_maxdet-p_ll_ens-3_rp-512_train",
            selection_method="maxdet",
            n_models=3,
            base_kernel="ll",
            kernel_transforms=[("ens", []), ("rp", [512]), ("train", [sigma, None])],
    ...
    for sel_name in ["maxdist", "kmeanspp", "lcmd"]:
        lst.append(
            2e-6,
            ModelTrainer(
                f"NN_{sel_name}-tp_linear",
                selection_method=sel_name,
                base_kernel="linear",
                kernel_transforms=[],
                **kwargs,
            ),
        ...
        lst.append(
            2e-6,
            ModelTrainer(
                f"NN_{sel_name}-tp_nngp", selection_method=sel_name, base_kernel="nngp", kernel_transforms=[], **kwargs
            ),
        )
        lst.append(
            4e-6,
            ModelTrainer(
                f"NN_{sel_name}-tp_ll", selection_method=sel_name, base_kernel="ll", kernel_transforms=[], **kwargs
            ),
        )
        lst.append(
            2e-5,
            ModelTrainer(
                f"NN_{sel_name}-tp_grad", selection_method=sel_name, base_kernel="grad", kernel_transforms=[], **kwargs
            ),
        )
        lst.append(
            4e-6,
            ModelTrainer(
                f"NN_{sel_name}-tp_grad_rp-512",
                selection_method=sel_name,
                base_kernel="grad",
        ...
        lst.append(
            8e-6,
            ModelTrainer(
                f"NN_{sel_name}-tp_grad_ens-3_rp-512",
                selection_method=sel_name,
                n_models=3,
                base_kernel="grad",
        ...
        lst.append(
            2e-5,
            ModelTrainer(
                f"NN_{sel_name}-p_grad_rp-512_train",
                selection_method=sel_name,
                sel_with_train=False,
                base_kernel="grad",
                kernel_transforms=[("rp", [512]), ("train", [sigma, None])],
        ...
        lst.append(
            1e-5,
            ModelTrainer(
                f"NN_{sel_name}-tp_ll_ens-3_rp-512",
                selection_method=sel_name,
                n_models=3,
                base_kernel="ll",
                kernel_transforms=[("ens", []), ("rp", [512])],
        ...
        lst.append(
            2e-5,
            ModelTrainer(
                f"NN_{sel_name}-p_grad_rp-512_acs-rf-512",
                selection_method=sel_name,
                sel_with_train=False,
                base_kernel="grad",
                kernel_transforms=[("rp", [512]), ("acs-rf", [512, sigma, None])],
        ...
        lst.append(
            2e-5,
            ModelTrainer(
                f"NN_{sel_name}-p_grad_rp-512_acs-grad",
                selection_method=sel_name,
                sel_with_train=False,
                base_kernel="grad",
                kernel_transforms=[("rp", [512]), ("acs-grad", [sigma, None])],
        ...
        lst.append(
            2e-5,
            ModelTrainer(
                f"NN_{sel_name}-p_grad_rp-512_acs-rf-hyper-512",
                selection_method=sel_name,
                sel_with_train=False,
                base_kernel="grad",
                kernel_transforms=[("rp", [512]), ("acs-rf-hyper", [512, None])],
        ...
        lst.append(
            2e-5,
            ModelTrainer(
                f"NN_{sel_name}-p_ll_acs-rf-512",
                selection_method=sel_name,
                sel_with_train=False,
                base_kernel="ll",
                kernel_transforms=[("acs-rf", [512, sigma, None])],
    ...
    lst.append(
        8e-6,
        ModelTrainer(
            "NN_maxdiag_grad_ens-3_rp-512_train",
            selection_method="maxdiag",
            base_kernel="grad",
            n_models=3,
            kernel_transforms=[("ens", []), ("rp", [512]), ("train", [sigma, None])],
    ...
    lst.append(
        1.3e-5,
        ModelTrainer(
            "NN_maxdiag_grad_rp-512_acs-grad",
            selection_method="maxdiag",
            base_kernel="grad",
            kernel_transforms=[("rp", [512]), ("acs-grad", [sigma, None])],
            **kwargs,
    ...
    lst.append(
        1.3e-5,
        ModelTrainer(
            "NN_maxdiag_grad_rp-512_acs-rf-hyper-512",
            selection_method="maxdiag",
            base_kernel="grad",
            kernel_transforms=[("rp", [512]), ("acs-rf-hyper", [512, None])],
            **kwargs,
    ...
    lst.append(
        2e-5,
        ModelTrainer(
            "NN_fw-p_ll_acs-grad_rp-512",
            selection_method="fw",
            base_kernel="ll",
            kernel_transforms=[("acs-grad", [sigma, None]), ("rp", [512])],
            **kwargs,
        ),
    )
    lst.append(
        2e-5,
        ModelTrainer(
            "NN_fw-p_grad_rp-512_acs-grad_rp-512",
            selection_method="fw",
            base_kernel="grad",
            kernel_transforms=[("rp", [512]), ("acs-grad", [sigma, None]), ("rp", [512])],
            **kwargs,
        ),
    )
    lst.append(
    ...
    lst.append(
        2e-5,
        ModelTrainer(
            "NN_fw-p_grad_rp-512_acs-rf-512",
            selection_method="fw",
            base_kernel="grad",
            kernel_transforms=[("rp", [512]), ("acs-rf", [512, sigma, None])],
            **kwargs,
    ...
    lst.append(
        2e-5,
        ModelTrainer(
            "NN_fw-p_grad_rp-512_acs-rf-hyper-512",
            selection_method="fw",
            base_kernel="grad",
            kernel_transforms=[("rp", [512]), ("acs-rf-hyper", [512, None])],
            **kwargs,
        ...
        for sigma_w in [0.25, 0.4, 0.7, 1.0, 1.414]:
            for wd in [1e-2, 1e-3, 0.0]:
                lst.append(
                    1e-6,
                    ModelTrainer(
                        f"NN_lr-{lr:g}_sigmaw-{sigma_w:g}_wd-{wd:g}",
                        lr=lr,
                        weight_decay=wd,
                        weight_gain=sigma_w,
        ...
        for sigma_b in [0.1, 0.25, 0.5, 1.0]:
            for lr in [3e-2 / sigma_w, 5e-2 / sigma_w, 7.5e-2 / sigma_w, 1e-1 / sigma_w]:
                ...
                lst.append(
                    1e-6,
                    ModelTrainer(
                        f"NN_sigmaw-{sigma_w:g}_wd-{wd:g}_sigmab-{sigma_b:g}_lr-{lr:g}",
                        lr=lr,
                        weight_decay=wd,
                        weight_gain=sigma_w,
                        weight_init_gain=wig,
...
def run_experiments(
    exp_name: str,
    n_splits: int,
    run_config_list: RunConfigList,
    batch_sizes_configs: list[list[int]] | None = None,
    task_descs: list[str] | None = None,
    use_pool_for_normalization: bool = True,
    max_jobs_per_device: int = 20,
    n_train_initial: int = 256,
    ds_names: list[str] | None = None,
    sequential_split: int | None = 9,
):
    """
    ...
    for max_split_id in range(0, n_splits):
        ...
        for split_id in range(0, max_split_id + 1):
            for batch_sizes_config, task_desc in zip(batch_sizes_configs, task_descs):
                tasks = Task.get_tabular_tasks(
                    n_train=n_train_initial, al_batch_sizes=batch_sizes_config, ds_names=ds_names, desc=task_desc
                )
