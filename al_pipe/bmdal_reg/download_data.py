...
from . import custom_paths, utils
...
class PandasTask:
    ...
    def __init__(
        ...
        y_df: pd.Series,
        ds_name: str,
        cat_indicator: Optional[List[bool]] = None,
        ...
        if cat_indicator is None:
            ...
            cat_indicator = [
                v or not pd.api.types.is_numeric_dtype(x_df[x_df.columns[i]]) for i, v in enumerate(cat_indicator)
            ]
    ...
    def subsample(self, max_tvp_samples: int, max_test_samples: int):
        ...
        if self.train_test_split is not None:
            dfs_train = self.subsample_dfs_(
                [self.x_df.loc[: self.train_test_split], self.y_df.loc[: self.train_test_split]],
                max_n_samples=max_tvp_samples,
            )
            dfs_test = self.subsample_dfs_(
                [self.x_df.loc[self.train_test_split :], self.y_df.loc[self.train_test_split :]],
                max_n_samples=max_test_samples,
            )
    ...
    def one_hot_encode(self, max_one_hot_columns: int):
        ...
        """
        cat_cols_with_size = [
            (col_name, self.x_df.loc[:, col_name].nunique()) for i, col_name in enumerate(self.cat_cols)
        ]
        ...
        for key, sz in cat_cols_with_size:
            if sz <= max_cat_size:
                ...
                dummies = pd.get_dummies(
                    col, prefix=f"{key}_onehot_", drop_first=(sz == 2), dummy_na=True, dtype=np.int32
                )
                ...
                print(
                    f"categorical column {key} with {sz} unique values is not one-hot encoded due to size constraints"
                )
    ...
    def from_openml_dataset_id(
        ...
        x_df, y_df, cat_indicator, names = dataset.get_data(target=target, dataset_format="dataframe")

        if ignore_columns is not None:
            cat_indicator = [
                value for col_name, value in zip(x_df.columns, cat_indicator) if col_name not in ignore_columns
            ]
    ...
    def from_uci(
        ...
        zip_name: str,
        csv_name: str,
        target_col_idxs: List[int],
        ignore_col_idxs: Optional[List[int]] = None,
        use_log_target: bool = False,
        train_test_boundary: Optional[int] = None,
        has_header: bool = True,
        continuous_nan_columns: Optional[List[int]] = None,
        convert_to_cat_columns: Optional[List[int]] = None,
        ignore_other_csv_files: bool = False,
        ...
        if len(non_train_files) > 0:
            ...
            df = pd.concat(
                [df]
                + [pd.read_csv(file, header="infer" if has_header else None, sep=separator) for file in non_train_files]
            )
        ...
        return PandasTask(x_df, y_df, ds_name, train_test_split=train_test_boundary)
...
class PandasTaskPreprocessor:
    ...
    def apply(self, pd_task: PandasTask):
        ...
        if pd_task.get_n_samples() < self.min_n_samples:
            print(
                f"Task {pd_task.ds_name} has only {pd_task.get_n_samples()} samples after removing missing values, "
...
def get_sarcos_pandas_task() -> PandasTask:
    ...
    """
    print("Importing dataset sarcos")
    base_path = custom_paths.get_data_path()
...
def import_all():
    proc = PandasTaskPreprocessor(
        min_n_samples=30000, max_tvp_samples=200000, max_test_samples=300000, max_one_hot_columns=300
    )
    ...
    proc.apply(
        PandasTask.from_uci(
            ...
            ds_name="sgemm",
            zip_name="sgemm+gpu+kernel+performance.zip",
            csv_name="sgemm_product.csv",
            target_col_idxs=[14, 15, 16, 17],
            use_log_target=True,
        )
    )
    proc.apply(
        PandasTask.from_uci(
            ...
            ds_name="ct",
            zip_name="relative+location+of+ct+slices+on+axial+axis.zip",
            csv_name="slice_localization_data.csv",
            target_col_idxs=[385],
            ignore_col_idxs=[0],
        )
    )
    ...
    proc.apply(
        PandasTask.from_uci(
            "https://archive.ics.uci.edu/static/public/221/kegg+metabolic+reaction+network+undirected.zip",
            ds_name="kegg_undir_uci",
            zip_name="kegg_undir_uci.zip",
            csv_name="Reaction Network (Undirected).data",
            target_col_idxs=[26],
            ignore_col_idxs=[0],
            has_header=False,
            continuous_nan_columns=[4],
        )
    )
    # only use the Sydney part of the data set (could as well have used Adelaide, Perth or Tasmania)
    proc.apply(
        PandasTask.from_uci(
            "https://archive.ics.uci.edu/ml/machine-learning-databases/00494/WECs_DataSet.zip",
            ds_name="wecs",
            zip_name="WECs_DataSet.zip",
            csv_name="WECs_DataSet/Sydney_Data.csv",
            target_col_idxs=[48],
            has_header=False,
            ignore_other_csv_files=True,
        )
    )
    proc.apply(
        PandasTask.from_uci(
            "https://archive.ics.uci.edu/ml/machine-learning-databases/00335/online_video_dataset.zip",
            ds_name="online_video",
            zip_name="online_video_dataset.zip",
            csv_name="transcoding_mesurment.tsv",
            separator="\t",
            ignore_col_idxs=[0, 20],
            target_col_idxs=[21],
            has_header=True,
            ignore_other_csv_files=True,
        )
    ...
    proc.apply(
        PandasTask.from_uci(
            "https://archive.ics.uci.edu/ml/machine-learning-databases/00493/datasets.zip",
            ds_name="query_agg_count",
            zip_name="datasets.zip",
            csv_name="Datasets/Range-Queries-Aggregates.csv",
            ignore_col_idxs=[0, 6, 7],
            target_col_idxs=[5],
            has_header=True,
            ignore_other_csv_files=True,
        )
    ...
    proc.apply(
        PandasTask.from_uci(
            ...
            ds_name="road_network",
            zip_name="3D_spatial_network.txt",
            csv_name="3D_spatial_network.txt",
            ignore_col_idxs=[0],
            target_col_idxs=[3],
            has_header=False,
            ignore_other_csv_files=True,
    ...
    proc.apply(
        PandasTask.from_openml_dataset_id(
            dataset_id=42454, ds_name="mlr_knn_rng", target="perf.logloss", ignore_columns=["perf.mmce"]
        )
    )
    proc.apply(
        PandasTask.from_openml_dataset_id(
            dataset_id=42701, ds_name="methane", target="MM264", ignore_columns=["MM263", "MM256"]
        )
    )
